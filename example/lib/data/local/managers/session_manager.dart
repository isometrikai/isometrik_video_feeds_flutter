...
class SessionManager {
  ...
  void saveIsLoggedIn(bool isLoggedIn) async {
    await _localStorageManager.saveValue(
        LocalStorageKeys.isLoggedIn, isLoggedIn, SavedValueDataType.bool);
  }
  ...
  void saveFirstName(String firstName) {
    _localStorageManager.saveValue(
        LocalStorageKeys.firstName, firstName, SavedValueDataType.string);
  }

  Future<String> getFirstName() async => await _localStorageManager.getValue(
      LocalStorageKeys.firstName, SavedValueDataType.string) as String;

  void saveFullName(String fullName) {
    _localStorageManager.saveValue(
        LocalStorageKeys.fullName, fullName, SavedValueDataType.string);
  }

  Future<String> getFullName() async => await _localStorageManager.getValue(
      LocalStorageKeys.fullName, SavedValueDataType.string) as String;

  void saveLastName(String lastName) {
    _localStorageManager.saveValue(
        LocalStorageKeys.lastName, lastName, SavedValueDataType.string);
  }

  Future<String> getLastName() async => await _localStorageManager.getValue(
      LocalStorageKeys.lastName, SavedValueDataType.string) as String;

  void saveProfilePic(String profile) {
    _localStorageManager.saveValue(
        LocalStorageKeys.profilePic, profile, SavedValueDataType.string);
  }

  void savePhoneNumber(String phoneNumber) {
    _localStorageManager.saveValueSecurely(
        LocalStorageKeys.phoneNumber, phoneNumber);
  }
  ...
  void saveCountryId(String countryId) {
    _localStorageManager.saveValue(
        LocalStorageKeys.countryId, countryId, SavedValueDataType.string);
  }

  Future<String> getCountryId() async => await _localStorageManager.getValue(
      LocalStorageKeys.countryId, SavedValueDataType.string) as String;

  void saveEmail(String email) {
    _localStorageManager.saveValue(
        LocalStorageKeys.email, email, SavedValueDataType.string);
  }

  Future<String> getEmail() async => await _localStorageManager.getValue(
      LocalStorageKeys.email, SavedValueDataType.string) as String;

  Future<String> getProfilePic() async => await _localStorageManager.getValue(
      LocalStorageKeys.profilePic, SavedValueDataType.string) as String;

  Future<String> getUserToken() async =>
      await _localStorageManager.getSecuredValue(LocalStorageKeys.accessToken);

  Future<void> saveUserToken(String userToken) async {
    await _localStorageManager.saveValueSecurely(
        LocalStorageKeys.accessToken, userToken);
  }

  Future<String> getRefreshToken() async =>
      await _localStorageManager.getSecuredValue(LocalStorageKeys.refreshToken);

  void saveRefreshToken(String userToken) {
    _localStorageManager.saveValueSecurely(
        LocalStorageKeys.refreshToken, userToken);
  }

  Future<String> getCurrencyCode() async => await _localStorageManager.getValue(
  ...
  Future<String> getCurrencySymbol() async =>
      await _localStorageManager.getValue(
          LocalStorageKeys.currencySymbol, SavedValueDataType.string) as String;

  void saveCurrencySymbol(String currencySymbol) {
    _localStorageManager.saveValue(LocalStorageKeys.currencySymbol,
        Utility.encodeChar(currencySymbol), SavedValueDataType.string);
  }
