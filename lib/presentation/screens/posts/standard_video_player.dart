...
class StandardVideoCacheManager implements IVideoCacheManager {
  ...
  /// Converts a Cloudinary video URL into MP4 format.
  /// Works with URLs like:
  /// https://res.cloudinary.com/<cloud_name>/video/upload/.../file.mkv
  String cloudinaryToMp4(String originalUrl) {
    if (!originalUrl.contains('/video/upload/')) return originalUrl;

    // Insert Cloudinary transformation `f_mp4` right after /upload/
    return originalUrl
        .replaceFirst(
          '/upload/',
          '/upload/f_mp4/',
        )
        .replaceAll(RegExp(r'\.mkv$|\.webm$|\.avi$|\.mov$'), '.mp4');
  }
}
